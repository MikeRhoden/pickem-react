...
describe('Event use cases (with all games selected and max units selected)', () => {
  ...
  test('Going over the unit limit should alert and prevent saving.', () => {
    ...
    expect(screen.getByText('Can\'t save. You are over 200 units.')).toBeInTheDocument()
  ...
  test('Matchup partially selected and saved should alert and prevent matchup save.', () => {
    ...
    expect(screen.getByText('Can\'t save. Please select units for the ' + proposition.matchup.visitor + ' vs ' + proposition.matchup.home + ' game.')).toBeInTheDocument()
    userEvent.click(screen.getByRole('button', { name: /ok/i }))
    ...
    userEvent.click(saveButton)
    expect(screen.getByText('Can\'t save. Please make a pick for the ' + proposition.matchup.visitor + ' vs ' + proposition.matchup.home + ' game.')).toBeInTheDocument()
    userEvent.click(screen.getByRole('button', { name: /ok/i }))
  ...
  test('Event loaded before matchup start and saved after matchup start should alert and disable change for matchup.', async () => {
    ...
    jest
      .spyOn(global.Date, 'now')
      .mockImplementation(() => pastEarlierStartTime.valueOf()
      );
  ...
  test('Event loaded before matchup start and saved after matchup start should alert and disable clear for matchup.', async () => {
    const propositions = MockPropositions(earliestLoadTime)
    render(<Event
      event={event}
      propositions={propositions}
...
function expectPropositionToBeDisabled(propositionElement) {
  ...
  expect(clearButtonThatShoudntExist).not.toBeInTheDocument()
}
...
function expectPropositionToBeEnabled(propositionElement) {
  ...
  const clearButtonThatShoudExist = propositionElement.querySelector('button')
  expect(clearButtonThatShoudExist).toBeInTheDocument()
}
